    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query){
                $deleteButton   = "<a href='".route('admin.product-gallery.destroy', $query->id)."' class='btn btn-danger ml-2 delete-item'><i class='far fa-trash-alt'></i></a>";

                return $deleteButton;
            })
            ->addColumn('images', function ($query){
                return "<img width='100' src='".asset($query->image)."'></img>";
            })
